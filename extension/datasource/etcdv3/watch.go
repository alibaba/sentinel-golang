...
func (w *Watch) update(resp *clientv3.WatchResponse) {
	...
	if err := resp.Err(); err != nil {
		logging.GetDefaultLogger().Error("etcd watch", w.client.Endpoints(), "err", err, "event", "update")
		return
	}
	...
	for _, event := range resp.Events {
		...
		default:
			logging.GetDefaultLogger().Info("etcd watch", w.client.Endpoints(), "event", "blocked")
		}
	}
}
