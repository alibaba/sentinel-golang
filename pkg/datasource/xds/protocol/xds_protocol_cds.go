...
func NewCdsProtocol(stopChan chan struct{}, updateChan chan resources.XdsUpdateEvent, xdsClientChannel *client.XdsClient) (*CdsProtocol, error) {
...
func (cds *CdsProtocol) ProcessProtocol(resp *v3discovery.DiscoveryResponse, xdsClientChannel *client.XdsClient) error {
	...
	for _, resource := range resp.GetResources() {
		...
		if cdsResource.GetType() == cluster.Cluster_EDS {
			...
			fmt.Printf("[CdsProtocol.ProcessProtocol] clusterName: %s, cds resource: %v\n", clusterName, cdsResource.String())
		}
	}
	...
	cds.xdsClientChannel.ApiStore.Store(client.EnvoyCluster, info)
	cds.xdsClientChannel.AckResponse(resp)

	if len(clusterNames) > 0 { // Load EDS
		cds.xdsClientChannel.ApiStore.SetResourceNames(client.EnvoyEndpoint, clusterNames)
